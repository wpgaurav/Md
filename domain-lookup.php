<?php
/**
 * Template Name: Domain Lookup Template
...
<div class="builder format">
  <div class="inner white-bg accordion block-single text-center mt-single">
  <h1 class="large-title block-single-tb">Domain Lookup!</h1>
    <form action="" id="domain-checker-form">
      <input type="text" name="domain" id="domain" value="<?=$domain;?>" placeholder="domain.com" title="Please Enter Only Domain Name"> 

  <input type="submit" value="Lookup" class="domain-checker-submit"></p>
  
</form>
 </div>
<div class="inner output white-bg accordion block-single">
<?php
if($domain) {
  $domain = trim($domain);
  if(substr(strtolower($domain), 0, 7) == "") $domain = substr($domain, 7);
  if(substr(strtolower($domain), 0, 4) == "") $domain = substr($domain, 4);
  ...
  else echo"<pre>Invalid Domain Name</pre>";
  echo "<pre>\n" . $result . "\n</pre>\n";
}
?>
</div>
